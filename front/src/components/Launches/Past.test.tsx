...
import { pastMock } from 'test/launchMockData'
...
it('should render the past launches', async () => {
  ...
  const pastLink = await screen.findByText(/PAST LAUNCH/i)
  ...
  const launchDateValue1 = screen.getByText(
    moment(pastMock[0].result.data.past[0].date_utc).format('DD/MM/yyyy'),
  )
  expect(launchDateValue1).toBeInTheDocument()
  const launchDateValue2 = screen.getByText(
    moment(pastMock[0].result.data.past[1].date_utc).format('DD/MM/yyyy'),
  )
  ...
  const launchHourValue1 = screen.getByText(
    moment(pastMock[0].result.data.past[0].date_utc).format('HH:mm'),
  )
  expect(launchHourValue1).toBeInTheDocument()
  const launchHourValue2 = screen.getByText(
    moment(pastMock[0].result.data.past[1].date_utc).format('HH:mm'),
  )
